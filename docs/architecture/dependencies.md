...
- local path dependencies must resolve to workspace members;
...
- `tracing` / `tracing-subscriber`: previously proved successfully at the Stage 7 application edge, but removed from the active foundation after that proof crate was retired. Re-admit at an executable/application edge when a real consumer needs structured operational tracing.
- dynamic loading/WASM/IPC libraries: select only after extension deployment/isolation requirements are concrete.

Git dependencies remain disallowed by current source policy unless an external-extension proof deliberately establishes a repository/revision trust model.
